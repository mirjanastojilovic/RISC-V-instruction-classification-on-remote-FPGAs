...
add_files -norecurse [glob $path_to_hdl/AxiFlusher.sv \
                           $path_to_hdl/AxiFifoFlusher.sv \
                           $path_to_hdl/AxiLoader.vhd \
                           $path_to_hdl/BramDumper.vhd \
                           $path_to_hdl/FifoDumper.vhd \
                           ...
                           $path_to_hdl/../../picorv32/rtl/picorv32_top.v \
                           $path_to_hdl/../../picorv32/rtl/picorv32.v \
                           $path_to_hdl/../../picorv32/rtl/memory.vhd]
...
if {$design eq "0"} {
  puts "USING SINGLE BANK DESIGN"
  if {$sensor eq "0"} {
    puts "USING TDC SENSOR"
    add_files -norecurse [glob $path_to_hdl/PicoRV32_SCA_kernel_TDC.vhd \
                               $path_to_hdl/AxiLiteFSM.vhd]
  } else {
    if {$sensor eq "1"} {
      puts "USING RDS SENSOR"
      add_files -norecurse [glob $path_to_hdl/PicoRV32_SCA_kernel_RDS.vhd \
                                 $path_to_hdl/AxiLiteFSM.vhd]
    } else {
      puts "USING TDC 6 SENSORS"
      add_files -norecurse [glob $path_to_hdl/PicoRV32_SCA_kernel_TDC6.vhd \
                                 $path_to_hdl/AxiLiteFSM.vhd]
    }
  }
} else {
  puts "USING MULTIPLE BANK DESIGN"
  add_files -norecurse [glob $path_to_hdl/bulk/PicoRV32_SCA_kernel.vhd \
                             $path_to_hdl/bulk/AxiLiteFSM.vhd \
                             $path_to_hdl/bulk/SensorBlock.vhd]
}
...
set_property -dict [list CONFIG.PRIM_IN_FREQ {200.000} CONFIG.CLKOUT2_USED {true} CONFIG.PRIMARY_PORT {axi_clk} CONFIG.CLK_OUT1_PORT {cpu_clk} CONFIG.CLK_OUT2_PORT {sens_clk} CONFIG.CLKOUT1_REQUESTED_OUT_FREQ {400.000} CONFIG.CLKOUT2_REQUESTED_OUT_FREQ {400.000} CONFIG.RESET_TYPE {ACTIVE_LOW} CONFIG.CLKIN1_JITTER_PS {50.0} CONFIG.MMCM_CLKFBOUT_MULT_F {4.000} CONFIG.MMCM_CLKIN1_PERIOD {5.000} CONFIG.MMCM_CLKIN2_PERIOD {10.0} CONFIG.MMCM_CLKOUT0_DIVIDE_F {2.000} CONFIG.MMCM_CLKOUT1_DIVIDE {2} CONFIG.NUM_OUT_CLKS {2} CONFIG.RESET_PORT {resetn} CONFIG.CLKOUT1_JITTER {87.396} CONFIG.CLKOUT1_PHASE_ERROR {97.786} CONFIG.CLKOUT2_JITTER {87.396} CONFIG.CLKOUT2_PHASE_ERROR {97.786}] [get_ips clock_generator]
