...
int main(int argc, char* argv[]) {
    ...
    if (argc != 10) {
        std::cerr << "usage: " << argv[0] << " XCLBIN N_SENSORS N_SAMPLES SENSOR_WIDTH IDC_SIZE IDF_SIZE SENSOR CALIB_TYPE PATH" << std::endl;
        printf("Error\n");
        std::exit(-1);
    }
    ...
    int SENSOR = atoi(argv[7]);
    int CALIB_TYPE = atoi(argv[8]);
    char * PATH = argv[9];
    ...
    if(CALIB_TYPE == 0 && SENSOR == 1) {
      ...
      idc_idf = calibrate_rds(kernel, buffer, hbuf, N_SENSORS, N_SAMPLES, IDC_SIZE, IDF_SIZE, SENSOR_WIDTH);
