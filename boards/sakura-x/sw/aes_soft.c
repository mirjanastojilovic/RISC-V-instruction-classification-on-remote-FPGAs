...
field with a higher polinomial degree than 7, we subtract (binary, with modulo 2 i.e. XOR) with the polinomial X^8+x^4+x^3+x^1+x^0
...
void KeyExpansion(unsigned char* inputKey, unsigned char* expandedKeys) {
  ...
  while (bytesGenerated < 176) {
    //Read last 4 bytes fenerated in the core
    for(int i=0; i<4; i++){
...
void MixColumns(unsigned char* state) {
  ...
  /*
  This function mutliplies the state with a special matrix.
...
void AES_Encrypt(unsigned char* message, unsigned char* key){
  ...
  //Copy encripted state to message:
  for(int i=0; i<16; i++) {
    message[i] = state[i];
  }
