...
def bilstm_resnet(X_train, y_train, X_test, y_test, epochs, batch_size, path, earlystop, learning_rate, run_id):
    ...
    bilstm = LSTM(128, return_sequences=True)(input_layer)

    conv_x = Conv1D(filters=n_feature_maps, kernel_size=8, padding='same')(bilstm)
    ...
    shortcut_y = Conv1D(filters=n_feature_maps, kernel_size=1, padding='same')(bilstm)
