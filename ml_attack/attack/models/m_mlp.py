...
def mlp(X_train, y_train, X_test, y_test, epochs, batch_size, path, earlystop, learning_rate, run_id):
    ...
    model = Sequential()
    model.add(Flatten())
    model.add(Dense(250, input_shape=(n_timesteps*n_features,)))
    ...
    # rate learning
    #reduce_lr = ReduceLROnPlateau(monitor='loss', factor=0.5, patience=50, min_lr=0.000001)
