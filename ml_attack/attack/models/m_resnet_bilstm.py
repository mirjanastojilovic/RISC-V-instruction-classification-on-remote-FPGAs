...
def resnet_bilstm(X_train, y_train, X_test, y_test, epochs, batch_size, path, earlystop, learning_rate, run_id):
    ...
    gap_layer = MaxPooling1D(pool_size=2)(output_block_3)

    # BILSTM block

    bilstm = LSTM(128, return_sequences=True)(gap_layer)

    # FINAL 
    
    flat = Flatten()(bilstm)
    output_layer = Dense(n_outputs, activation='softmax')(flat)
