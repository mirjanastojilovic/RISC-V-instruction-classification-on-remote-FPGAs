...
def resnet_v2(X_train, y_train, X_test, y_test, epochs, batch_size, path, earlystop, learning_rate, run_id):
    ...
    conv_x = Conv1D(filters=n_feature_maps, kernel_size=12, padding='same')(input_layer)
    conv_x = BatchNormalization()(conv_x)
    conv_x = Activation('relu')(conv_x)

    conv_y = Conv1D(filters=n_feature_maps, kernel_size=8, padding='same')(conv_x)
    conv_y = BatchNormalization()(conv_y)
    conv_y = Activation('relu')(conv_y)

    conv_y1 = Conv1D(filters=n_feature_maps, kernel_size=5, padding='same')(conv_y)
    conv_y1 = BatchNormalization()(conv_y1)
    conv_y1 = Activation('relu')(conv_y1)

    conv_z = Conv1D(filters=n_feature_maps, kernel_size=3, padding='same')(conv_y1)
    ...
    conv_x = Conv1D(filters=n_feature_maps*2, kernel_size=12, padding='same')(output_block_1)
    conv_x = BatchNormalization()(conv_x)
    conv_x = Activation('relu')(conv_x)

    conv_y = Conv1D(filters=n_feature_maps*2, kernel_size=8, padding='same')(conv_x)
    conv_y = BatchNormalization()(conv_y)
    conv_y = Activation('relu')(conv_y)

    conv_y1 = Conv1D(filters=n_feature_maps*2, kernel_size=5, padding='same')(conv_y)
    conv_y1 = BatchNormalization()(conv_y1)
    conv_y1 = Activation('relu')(conv_y1)

    conv_z = Conv1D(filters=n_feature_maps*2, kernel_size=3, padding='same')(conv_y1)
    conv_z = BatchNormalization()(conv_z)
    ...
    conv_x = Conv1D(filters=n_feature_maps*2, kernel_size=12, padding='same')(output_block_2)
    conv_x = BatchNormalization()(conv_x)
    conv_x = Activation('relu')(conv_x)
